import math
import random

# Load Gtk, Gdk, GLib
import gi
gi.require_version('Gtk', '4.0')
from gi.repository import Gtk, Gdk, GLib

frames = 0
score = 0
game_layer = None


BUFFER_W = 320
BUFFER_H = 240

DISP_SCALE = 2
DISP_W = BUFFER_W * DISP_SCALE
DISP_H = BUFFER_H * DISP_SCALE

SHIP_W = 12 * DISP_SCALE
SHIP_H = 13 * DISP_SCALE

SHIP_SHOT_W = 2 * DISP_SCALE
SHIP_SHOT_H = 9 * DISP_SCALE

LIFE_W = 6 * DISP_SCALE
LIFE_H = 6 * DISP_SCALE

ALIEN_W = [14 * DISP_SCALE, 13 * DISP_SCALE, 45 * DISP_SCALE]
ALIEN_H = [9 * DISP_SCALE, 10 * DISP_SCALE, 27 * DISP_SCALE]

ALIEN_BUG_W      = ALIEN_W[0]
ALIEN_BUG_H      = ALIEN_H[0]
ALIEN_ARROW_W    = ALIEN_W[1]
ALIEN_ARROW_H    = ALIEN_H[1]
ALIEN_THICCBOI_W = ALIEN_W[2]
ALIEN_THICCBOI_H = ALIEN_H[2]

ALIEN_SHOT_W = 4 * DISP_SCALE
ALIEN_SHOT_H = 4 * DISP_SCALE

EXPLOSION_FRAMES = 4
SPARKS_FRAMES    = 3

KEY_SEEN = 1
KEY_RELEASED = 2
key = {
  Gdk.KEY_Left: 0,
  Gdk.KEY_Right: 0,
  Gdk.KEY_Up: 0,
  Gdk.KEY_Down: 0,
  Gdk.KEY_space: 0,
}


def collide(ax1, ay1, ax2, ay2, bx1, by1, bx2, by2):
    if ax1 > bx2: 
        return False
    if ax2 < bx1:
        return False
    if ay1 > by2:
        return False
    if ay2 < by1:
        return False

    return True

#--- sprites ---
sprites = {  
    "ship_shot": [0, 1],
    "life": 0,
  
    "alien": [0, 1, 2],
    "alien_shot": 0,
  
    "explosion": [0, 1, 2, 3],
    "sparks": [0, 1, 2],
  
    "star": 0,
}

def sprite_new(sprite_name):
    return Gdk.Texture.new_from_filename("sprites/" + sprite_name + ".png")

def sprites_init():
    global sprites

    sprites["life"] = sprite_new("life")

    sprites["ship_shot"][0] = sprite_new("shipshot0")
    sprites["ship_shot"][1] = sprite_new("shipshot1")

    sprites["alien"][0] = sprite_new("alienbug")
    sprites["alien"][1] = sprite_new("alienarrow")
    sprites["alien"][2] = sprite_new("alienthiccboi")

    sprites["alien_shot"] = sprite_new("alienshot")

    sprites["explosion"][0] = sprite_new("explosion0")
    sprites["explosion"][1] = sprite_new("explosion1")
    sprites["explosion"][2] = sprite_new("explosion2")
    sprites["explosion"][3] = sprite_new("explosion3")

    sprites["sparks"][0] = sprite_new("sparks0")
    sprites["sparks"][1] = sprite_new("sparks1")
    sprites["sparks"][2] = sprite_new("sparks2")
  
    sprites["star"] = sprite_new("star")

#--- fx ---
FX_N = 128
fx = []

def fx_init():
    global FX_N, fx, game_layer
    for i in range(FX_N):
        a_fx = {
          "used": False,
          "picture": Gtk.Picture(),
        }
        game_layer.put(a_fx["picture"], 0, 0)
        fx.append(a_fx)

def fx_add(spark, x, y):
    global FX_N, fx
    for i in range(FX_N):
        a_fx = fx[i]
        if not a_fx["used"]:
          a_fx["x"] = x
          a_fx["y"] = y
          a_fx["frame"] = 0
          a_fx["spark"] = spark
          a_fx["used"] = True
          a_fx["picture"].props.visible = True
          return

def fx_update():
    global FX_N, fx, EXPLOSION_FRAMES, SPARKS_FRAMES
    for i in range(FX_N):
        a_fx = fx[i]
        if a_fx["used"]:
            a_fx["frame"] = a_fx["frame"] + 1

            if ((not a_fx["spark"] and a_fx["frame"] == EXPLOSION_FRAMES * 2) or
                (a_fx["spark"] and a_fx["frame"] == SPARKS_FRAMES * 2)):
                a_fx["used"] = False
                a_fx["picture"].props.visible = False


def fx_draw():
    global FX_N, fx, sprites, game_layer, DISP_SCALE
    for i in range(FX_N):
        a_fx = fx[i]
        if a_fx["used"]:
            frame_display = math.floor(a_fx["frame"] / 2)
            bmp = sprites["sparks"][frame_display] if a_fx["spark"] else sprites["explosion"][frame_display]
      
            width = bmp.get_width() * DISP_SCALE
            height = bmp.get_height() * DISP_SCALE
            x = a_fx["x"] - math.floor(width / 2)
            y = a_fx["y"] - math.floor(height / 2)
      
            picture = a_fx["picture"]
            picture.set_paintable(bmp)
            picture.set_size_request(width, height)
            game_layer.move(picture, x, y)

#--- shots ---
SHOTS_N = 128
shots = []

def shots_init():
    global SHOTS_N, shots, game_layer
    for i in range(SHOTS_N):
        shot = {
          "used": False,
          "picture": Gtk.Picture(),
        }
        shots.append(shot)
        game_layer.put(shot["picture"], 0, 0)

def shots_add(ship, straight, x, y):
    global SHOTS_N, shots, SHIP_SHOT_W, ALIEN_SHOT_W, ALIEN_SHOT_H
    for i in range(SHOTS_N):
        shot = shots[i]
        if not shot["used"]:
            shot["ship"] = ship

            if ship:
                shot["x"] = x - math.floor(SHIP_SHOT_W / 2)
                shot["y"] = y
            else: # alien
                shot["x"] = x - math.floor(ALIEN_SHOT_W / 2)
                shot["y"] = y - math.floor(ALIEN_SHOT_H / 2)

                if straight:
                    shot["dx"] = 0
                    shot["dy"] = 4
                else:
                    shot["dx"] = random.randint(-4, 4) 
                    shot["dy"] = random.randint(-4, 4) 

                # if the shot has no speed, don't bother
                if shot["dx"] == 0 and shot["dy"] == 0:
                    return True
                
                shot["frame"] = 0

            shot["frame"] = 0
            shot["used"] = True
            shot["picture"].props.visible = True
            return True
    
    return False

def shots_update():
    global SHOTS_N, shots, ALIEN_SHOT_W, DISP_W, ALIEN_SHOT_H, DISP_H, SHIP_SHOT_H

    for i in range(SHOTS_N):
        shot = shots[i]
        if not shot["used"]:
            continue
        
        picture = shot["picture"]
          
        if shot["ship"]:
            shot["y"] = shot["y"] - 10

            if shot["y"] < -SHIP_SHOT_H:
                shot["used"] = False
                picture.props.visible = False
                continue
        else: # alien
            shot["x"] = shot["x"] + shot["dx"]
            shot["y"] = shot["y"] + shot["dy"]

            if (shot["x"] < -ALIEN_SHOT_W or
              shot["x"] > DISP_W or
              shot["y"] < -ALIEN_SHOT_H or
              shot["y"] > DISP_H):
            
                shot["used"] = False
                picture.props.visible = False
                continue

        shot["frame"] = shot["frame"] + 1

def shots_collide(ship, x, y, w, h):
    global SHOTS_N, shots, ALIEN_SHOT_W, ALIEN_SHOT_H, SHIP_SHOT_W, SHIP_SHOT_H

    for i in range(SHOTS_N):
        shot = shots[i]
        if not shot["used"]:
            continue

        # don't collide with one's own shots
        if shot["ship"] == ship:
            continue

        sw = 0 
        sh = 0
        if ship:
            sw = ALIEN_SHOT_W
            sh = ALIEN_SHOT_H
        else:
            sw = SHIP_SHOT_W
            sh = SHIP_SHOT_H

        if collide(x, y, x+w, y+h, shot["x"], shot["y"], shot["x"]+sw, shot["y"]+sh):
            fx_add(True, shot["x"] + math.floor(sw / 2), shot["y"] + math.floor(sh / 2))
            shot["used"] = False
            shot["picture"].props.visible = False
            return True

    return False

def shots_draw():
    global SHOTS_N, shots, SHIP_SHOT_W, SHIP_SHOT_H, ALIEN_SHOT_W, ALIEN_SHOT_H, sprites, game_layer

    for i in range(SHOTS_N):
        shot = shots[i]
        if shot["used"]:
            frame_display = math.floor(shot["frame"] / 2) % 2
            picture = shot["picture"]
            if shot["ship"]:
                picture.set_paintable(sprites["ship_shot"][frame_display])
                picture.set_size_request(SHIP_SHOT_W, SHIP_SHOT_H)
                game_layer.move(picture, shot["x"], shot["y"])
            else:# alien
                picture.set_paintable(sprites["alien_shot"])
                picture.set_size_request(ALIEN_SHOT_W, ALIEN_SHOT_H)
                game_layer.move(picture, shot["x"], shot["y"])


#--- ship ---
SHIP_SPEED = 3 * DISP_SCALE
SHIP_MAX_X = DISP_W - SHIP_W
SHIP_MAX_Y = DISP_H - SHIP_H

ship = {}

def ship_init():
    global ship, SHIP_W, SHIP_H, DISP_W, DISP_H, game_layer

    ship["picture"] = Gtk.Picture.new_for_filename("sprites/ship.png")
    ship["picture"].set_size_request(SHIP_W, SHIP_H)
    
    ship["x"] = math.floor(DISP_W / 2) - math.floor(SHIP_W / 2)
    ship["y"] = math.floor(DISP_H / 2) - math.floor(SHIP_H / 2)
    game_layer.put(ship["picture"], ship["x"], ship["y"])
    
    ship["shot_timer"] = 0
    ship["lives"] = 3
    ship["respawn_timer"] = 0
    ship["invincible_timer"] = 120

def ship_update():
    global ship, key, SHIP_SPEED, SHIP_MAX_X, SHIP_MAX_Y, SHIP_W, SHIP_H
    if ship["lives"] < 0:
        return

    if ship["respawn_timer"] > 0:
        ship["respawn_timer"] = ship["respawn_timer"] - 1
        return

    if key[Gdk.KEY_Left] > 0:
        ship["x"] = ship["x"] - SHIP_SPEED
    
    if key[Gdk.KEY_Right] > 0:
        ship["x"] = ship["x"] + SHIP_SPEED
   
    if key[Gdk.KEY_Up] > 0:
        ship["y"] = ship["y"] - SHIP_SPEED
   
    if key[Gdk.KEY_Down] > 0:
        ship["y"] = ship["y"] + SHIP_SPEED

    if ship["x"] < 0:
        ship["x"] = 0
   
    if ship["y"] < 0:
        ship["y"] = 0
  
    if ship["x"] > SHIP_MAX_X:
        ship["x"] = SHIP_MAX_X
    
    if ship["y"] > SHIP_MAX_Y:
        ship["y"] = SHIP_MAX_Y

    if ship["invincible_timer"] > 0:
        ship["invincible_timer"] = ship["invincible_timer"] - 1
    else:
        if shots_collide(True, ship["x"], ship["y"], SHIP_W, SHIP_H):
            x = ship["x"] + math.floor(SHIP_W / 2)
            y = ship["y"] + math.floor(SHIP_H / 2)
            fx_add(False, x, y)
            fx_add(False, x+8, y+4)
            fx_add(False, x-4, y-8)
            fx_add(False, x+2, y-10)

            ship["lives"] = ship["lives"] - 1
            ship["respawn_timer"] = 90
            ship["invincible_timer"] = 180

    if ship["shot_timer"] > 0:
        ship["shot_timer"] = ship["shot_timer"] - 1
    elif key[Gdk.KEY_space] > 0:
        x = ship["x"] + math.floor(SHIP_W / 2)
        if shots_add(True, False, x, ship["y"]):
            ship["shot_timer"] = 5


def ship_draw():
    global ship, game_layer

    if ship["lives"] < 0:
        ship["picture"].props.visible = False
        return

    if ship["respawn_timer"] > 0:
        ship["picture"].props.visible = False
        return
   
    if (math.floor(ship["invincible_timer"] / 2) % 3) == 1:
        ship["picture"].props.visible = False
        return
    
    game_layer.move(ship["picture"], ship["x"], ship["y"])
    ship["picture"].props.visible = True


#--- aliens ---
ALIEN_TYPE_BUG = 0
ALIEN_TYPE_ARROW = 1
ALIEN_TYPE_THICCBOI = 2
ALIEN_TYPE_N = 3

ALIENS_N = 16
aliens = []

def aliens_init():
    global aliens, ALIENS_N, game_layer
    for i in range(ALIENS_N):
        alien = {
          "used": False,
          "picture": Gtk.Picture()
        }
        game_layer.put(alien["picture"], 0, 0)
        aliens.append(alien)

def aliens_update():
    global frames, aliens, score, ALIENS_N, DISP_W, DISP_H, ALIEN_TYPE_N, ALIEN_TYPE_BUG, ALIEN_TYPE_ARROW, ALIEN_TYPE_THICCBOI, ALIEN_W, ALIEN_H

    new_quota = 0 if (frames % 120) > 0 else random.randint(2, 4)
    new_x = random.randint(20, DISP_W-100)

    for i in range(ALIENS_N):
        alien = aliens[i]
        if not alien["used"]:
            # if this alien is unused, should it spawn?
            if new_quota > 0:
                new_x = new_x + random.randint(80, 160)
                if new_x > (DISP_W - 120):
                    new_x = new_x - (DISP_W - 120)

                alien["x"] = new_x

                alien["y"] = random.randint(-80, -60)
                alien["type"] = random.randrange(ALIEN_TYPE_N)
                alien["shot_timer"] = random.randint(1, 99)
                alien["blink"] = 0
                alien["used"] = True
                alien["picture"].props.visible = True

                if alien["type"] == ALIEN_TYPE_BUG:
                    alien["life"] = 4
                elif alien["type"] == ALIEN_TYPE_ARROW:
                    alien["life"] = 2
                elif alien["type"] == ALIEN_TYPE_THICCBOI:
                    alien["life"] = 12

                new_quota = new_quota - 1
              
            continue

        if alien["type"] == ALIEN_TYPE_BUG:
            if (frames % 2) > 0:
                alien["y"] = alien["y"] + 2

        elif alien["type"] == ALIEN_TYPE_ARROW:
            alien["y"] = alien["y"] + 2

        elif alien["type"] == ALIEN_TYPE_THICCBOI:
            if (frames % 4) == 0:
              alien["y"] = alien["y"] + 2

        if alien["y"] >= DISP_H:
            alien["used"] = False
            alien["picture"].props.visible = False
            continue

        if alien["blink"] > 0:
          alien["blink"] = alien["blink"] - 1
            

        if shots_collide(False, alien["x"], alien["y"], ALIEN_W[alien["type"]], ALIEN_H[alien["type"]]):
            alien["life"] = alien["life"] - 1
            alien["blink"] = 4

        cx = alien["x"] + math.floor(ALIEN_W[alien["type"]] / 2)
        cy = alien["y"] + math.floor(ALIEN_H[alien["type"]] / 2)

        if alien["life"] <= 0:
            fx_add(False, cx, cy)

            if alien["type"] == ALIEN_TYPE_BUG:
                score = score + 200
            elif alien["type"] == ALIEN_TYPE_ARROW:
                score = score + 150
            elif alien["type"] == ALIEN_TYPE_THICCBOI:
                score = score + 800
                fx_add(False, cx-20, cy-8)
                fx_add(False, cx+8, cy+20)
                fx_add(False, cx+16, cy+16)

            alien["used"] = False
            alien["picture"].props.visible = False
            continue

        alien["shot_timer"] = alien["shot_timer"] - 1
        if alien["shot_timer"] == 0:
            if alien["type"] == ALIEN_TYPE_BUG:
                shots_add(False, False, cx, cy)
                alien["shot_timer"] = 150
            elif alien["type"] == ALIEN_TYPE_ARROW:
                shots_add(False, True, cx, alien["y"])
                alien["shot_timer"] = 80
            elif alien["type"] == ALIEN_TYPE_THICCBOI:
                shots_add(False, True, cx-10, cy)
                shots_add(False, True, cx+10, cy)
                shots_add(False, True, cx-10, cy + 16)
                shots_add(False, True, cx+10, cy + 16)
                alien["shot_timer"] = 200

def aliens_draw():
    global aliens, ALIENS_N, ALIEN_W, ALIEN_H, game_layer, sprites

    for i in range(ALIENS_N):
        alien = aliens[i]
        picture = alien["picture"]
        
        if not alien["used"]:
            picture.props.visible = False
            continue

        if alien["blink"] > 2:
            picture.props.visible = False
            continue

        
        #-- add red tint to alien ships when being hit
        if alien["blink"] > 0 and not alien["picture"].has_css_class("red"):
            alien["picture"].add_css_class("red")
        elif alien["picture"].has_css_class("red"):
            alien["picture"].remove_css_class("red")
        
        
        picture.set_paintable(sprites["alien"][alien["type"]])
        picture.set_size_request(ALIEN_W[alien["type"]], ALIEN_H[alien["type"]])
        game_layer.move(picture, alien["x"], alien["y"])
        alien["picture"].props.visible = True


#--- HUD ---
score_display = 0
score_display_changed = False
score_label = None

gameover_label = None
lives_pictures = []

def hud_init():
    global score_label, score_display, game_layer, LIFE_W, LIFE_H

    score_label = Gtk.Label(label='<span foreground="white">{:06d}</span>'.format(score_display), use_markup=True)
    game_layer.put(score_label, 2, 2)
    
    spacing = LIFE_W + 2
    for i in range(3):
        picture = Gtk.Picture.new_for_filename("sprites/life.png")
        picture.set_size_request(LIFE_W, LIFE_H)
        game_layer.put(picture, 2 + (i * spacing), 20)
        lives_pictures.append(picture)

def hud_update():
    global frames, score_display, score_display_changed, score

    if (frames % 2) > 0:
        return
    
    for i in range(5, 0, -1):
        diff = 1 << i
        if score_display <= (score - diff):
            score_display = score_display + diff
            score_display_changed = True


def hud_draw():
    global score_display_changed, score_label, score_display, ship, lives_pictures

    if score_display_changed:
        score_label.set_label('<span foreground="white">{:06d}</span>'.format(score_display))
        score_display_changed = False
    
    start_i = ship["lives"]
    if start_i < 0:
        start_i = 0
    for i in range(start_i, 3):
        lives_pictures[i].props.visible = False

    if ship["lives"] < 0:
        gameover_label.props.visible = True


#--- stars ---
STARS_N = math.floor(BUFFER_W / 2) - 1
stars = []

def stars_init():
    global stars, STARS_N, sprites, game_layer

    for i in range (STARS_N):
        star = {
            "y": random.uniform(0, DISP_H), 
            "speed": random.uniform(0.2, 2),
            "picture": Gtk.Picture.new_for_paintable(sprites["star"]),
        }
        star_picture = star["picture"]
        star_picture.set_size_request(2, 2)
        game_layer.put(star_picture, -4, -4)
        stars.append(star)

def stars_update():
    global stars, STARS_N

    for i in range(STARS_N):
        star = stars[i]
        star["y"] = star["y"] + star["speed"]
        if star["y"] >= DISP_H:
            star["y"] = 0
            star["speed"] = random.uniform(0.2, 2)

def stars_draw():
    global stars, STARS_N, game_layer

    star_x = 1.5
    for i in range(STARS_N):
        star = stars[i]
        game_layer.move(star["picture"], star_x, star["y"])
        star_x = star_x + 4

#--- game loop ---
window = None
timer_id = 0

def game_loop():
    global frames, key, KEY_SEEN

    stars_update()
    fx_update()
    shots_update()
    ship_update()
    aliens_update()
    hud_update()

    frames = frames + 1
  
    for k, v in key.items():
        key[k] = key[k] & KEY_SEEN
  
    stars_draw()
    aliens_draw()
    shots_draw()
    fx_draw()
    ship_draw()
    hud_draw()
    return True


#--- keyboard ---

def on_keypressed(controller, keyval, keycode, state, user_data):
    global key, KEY_SEEN, KEY_RELEASED, window

    key[keyval] = KEY_SEEN + KEY_RELEASED
    
    if keyval == Gdk.KEY_Escape:
      window.close()


def on_keyreleased(controller, keyval, keycode, state, user_data):
    global key, KEY_RELEASED
    key[keyval] = key[keyval] & KEY_RELEASED


def on_close(window, user_data):
    global timer_id
    if timer_id > 0:
        GLib.Source.remove(timer_id)
        print("Timer stopped")

    timer_id = 0
    print("on_close")
    return False
    
css_provider = None

def on_activate(app):
    global window, timer_id, game_layer, gameover_label, css_provider

    window = Gtk.ApplicationWindow(application=app)
    window.set_title("PyGObject Gtk4 Shooting Game")
    window.set_default_size(DISP_W, DISP_H)
    window.set_resizable(False)
    
    overlay = Gtk.Overlay()
    overlay.set_focusable(True)
    
    bg_picture = Gtk.Picture.new_for_filename("sprites/bg_pixel.png")
    bg_picture.set_size_request(DISP_W, DISP_H)
    overlay.set_child(bg_picture)

    game_layer = Gtk.Fixed()
    overlay.add_overlay(game_layer)

    vbox = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10,
                   halign=Gtk.Align.CENTER, valign=Gtk.Align.CENTER)
    overlay.add_overlay(vbox)

    gameover_label = Gtk.Label(label='<span foreground="white">G A M E  O V E R</span>',
                               use_markup=True)
    vbox.append(gameover_label)
    gameover_label.props.visible = False
    
    window.set_child(overlay)

    #--- Game Main ---
    sprites_init()
  
    stars_init()
    aliens_init()
    shots_init()
    fx_init()
    ship_init()
    hud_init()

    keyboard = Gtk.EventControllerKey()
    overlay.add_controller(keyboard)
    keyboard.connect("key-pressed", on_keypressed, overlay)
    keyboard.connect("key-released", on_keyreleased, overlay)
    overlay.connect("destroy", on_close)

    fps = 60
    timer_id = GLib.timeout_add(math.floor(1000/fps), game_loop)

    #--- add css for tint effects ---
    css_provider = Gtk.CssProvider()
    css_str = b".red{filter: sepia(100%) hue-rotate(0deg) saturate(10000%) brightness(75%);}"
    css_provider.load_from_data(css_str)
    Gtk.StyleContext.add_provider_for_display(Gdk.Display.get_default(), css_provider, 800)

    window.present()

app = Gtk.Application()
app.connect('activate', on_activate)
app.run(None)
